...
async function ensureMigrationsTable(conn: ReturnType<typeof connect>): Promise<void> {
  // Bootstrap the tracking table if this is the very first run.
  // Migration 001 also creates it, but we need the table to exist
  // before we can query which migrations have been applied.
  await conn.execute(`
...
async function applyMigration(
  ...
  // Filter out empty strings from trailing semicolons.
