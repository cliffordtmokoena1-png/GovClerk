...
CREATE TABLE IF NOT EXISTS gc_portal_subscriptions (
  ...
  status                        ENUM('pending_activation','active','trial','suspended','cancelled') NOT NULL DEFAULT 'trial',
  paystack_subscription_code    VARCHAR(255)  NULL,
  paystack_plan_code            VARCHAR(255)  NULL,
  paystack_authorization_code   VARCHAR(255)  NULL COMMENT 'Authorization code from pro-rata transaction',
  preferred_billing_day         TINYINT       NULL COMMENT '1–28, chosen by client at sign-up',
  prorata_amount_zar            DECIMAL(10,2) NULL COMMENT 'First-period pro-rata charge in ZAR',
  prorata_tokens                INT           NULL COMMENT 'Pro-rated tokens credited in first period (Professional only)',
  prorata_paid_at               TIMESTAMP     NULL COMMENT 'When the pro-rata charge was successfully collected',
  activation_scheduled_at       TIMESTAMP     NULL COMMENT 'When the recurring subscription was created in Paystack',
...
CREATE TABLE IF NOT EXISTS gc_portal_quote_requests (
  ...
  preferred_billing_day                 TINYINT      NULL COMMENT '1–28, preferred billing day chosen by client',
