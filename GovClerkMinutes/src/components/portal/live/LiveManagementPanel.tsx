...
function RollCallTab({
  ...
  return (
    <div className="space-y-4">
      <div
        className={`p-3 rounded-lg border text-sm font-medium ${hasQuorum ? "bg-green-50 border-green-200 text-green-800" : "bg-yellow-50 border-yellow-200 text-yellow-800"}`}
      >
    ...
    try {
      await fetch(`/api/portal/live/${broadcastId}/motions`, {
        ...
        body: JSON.stringify({
          title: newTitle.trim(),
          motionType: newType,
          movedBy: newMovedBy || undefined,
          secondedBy: newSecondedBy || undefined,
        }),
      });
    ...
    <div className="space-y-4">
      ...
      {showNewForm && (
        <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg space-y-3">
          ...
          >
            {["motion", "resolution", "ordinance", "bylaw", "amendment", "procedural"].map(
              (motionType) => (
                <option key={motionType} value={motionType}>
                  {motionType.charAt(0).toUpperCase() + motionType.slice(1)}
                </option>
              )
            )}
          </select>
          ...
          <input
            type="text"
            value={newMovedBy}
            onChange={(e) => setNewMovedBy(e.target.value)}
          ...
          <input
            type="text"
            value={newSecondedBy}
            onChange={(e) => setNewSecondedBy(e.target.value)}
        ...
        <div key={motion.id} className="p-4 bg-white border border-gray-200 rounded-xl space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {motion.status === "pending" && (
              <button
                onClick={() => updateMotionStatus(motion.id, "open")}
            ...
            {motion.status === "open" && (
              <>
                <button
                  onClick={() => updateMotionStatus(motion.id, "passed")}
                  className="px-3 py-1.5 text-xs bg-green-600 text-white rounded-lg hover:bg-green-700"
                >
                  Mark Passed
                </button>
                <button
                  onClick={() => updateMotionStatus(motion.id, "failed")}
                  className="px-3 py-1.5 text-xs bg-red-600 text-white rounded-lg hover:bg-red-700"
                >
                  Mark Failed
                </button>
                <button
                  onClick={() => updateMotionStatus(motion.id, "tabled")}
            ...
            <button
              onClick={() => updateMotionStatus(motion.id, "withdrawn")}
  ...
  return (
    <div className="space-y-4">
      ...
      {speakerQueue.map((entry) => (
        <div
          key={entry.id}
          className={`flex items-center gap-3 p-3 rounded-xl border ${entry.status === "speaking" ? "bg-green-50 border-green-300" : "bg-white border-gray-200"}`}
        >
          <span
            className={`w-6 h-6 rounded-full text-xs flex items-center justify-center font-bold ${entry.status === "speaking" ? "bg-green-500 text-white" : "bg-gray-100 text-gray-500"}`}
          >
            {entry.position}
          </span>
          ...
          <div className="flex gap-1">
            {entry.status === "waiting" && (
              <button
                onClick={() => updateStatus(entry.id, "speaking")}
          ...
          <div className="space-y-2">
            {pending.map((comment) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => updateComment(comment.id, "approved", approved.length + 1)}
        ...
        <div>
          ...
          <div className="space-y-2">
            {approved.map((comment, index) => (
              ...
              >
                <span className="w-5 h-5 rounded-full bg-blue-200 text-blue-700 text-xs flex items-center justify-center font-bold">
                  {index + 1}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900">{comment.speakerName}</p>
                ...
                <button
                  onClick={() => updateComment(comment.id, "spoken")}
                  className="px-2 py-1 text-xs bg-gray-600 text-white rounded"
                >
                  Spoken
                </button>
...
export function LiveManagementPanel({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      <div className="p-4">
        {activeTab === "rollcall" && (
          <RollCallTab broadcastId={broadcastId} attendance={attendance} onRefresh={onRefresh} />
        )}
        {activeTab === "motions" && (
