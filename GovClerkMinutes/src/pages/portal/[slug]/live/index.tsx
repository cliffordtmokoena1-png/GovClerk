/**
 * Public Portal — Live Page (/portal/[slug]/live)
 *
 * Shown for organisations with an active subscription.
 * Only accessible to authenticated users with an active or trial subscription
 * (or GovClerk admins). All others are redirected to /portal/[slug]/demo.
 */
...
export default function LivePortalPage({
  ...
  const allMeetings = useMemo(() => initialMeetings.meetings, [initialMeetings.meetings]);
...
export const getServerSideProps: GetServerSideProps<LivePortalPageProps> = async (context) => {
  ...
  // Unauthenticated users cannot access the live portal — send to demo
  if (!session) {
    return {
      redirect: {
  ...
  // GovClerk admins (emails ending @govclerkminutes.com) always access the live portal.
  ...
  // If no active subscription, redirect to demo
  if (portalMode === "demo") {
    return {
      redirect: {
        destination: `/portal/${slug}/demo`,
  ...
  // Fetch live portal content in parallel
  try {
    ...
    console.error("Error fetching live portal data:", error);
    return {
      props: {
        settings: settingsData.settings,
