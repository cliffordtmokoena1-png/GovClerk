...
function ComplianceBadge({ isCompliant }: { isCompliant: boolean | null }) {
  ...
  return isCompliant ? (
    <span className="inline-flex items-center gap-1 text-xs text-green-700" aria-label="Notice compliant">
      ✅ <span>Compliant</span>
    </span>
  ) : (
    <span className="inline-flex items-center gap-1 text-xs text-yellow-700" aria-label="Late notice">
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={`Meeting calendar for ${settings.pageTitle || "our organization"}`} />
      </Head>
      ...
      <main id="main-content" className="max-w-5xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <nav aria-label="Breadcrumb" className="mb-4 text-sm text-gray-500">
          <ol className="flex items-center gap-2">
            ...
            <li aria-current="page" className="text-gray-800 font-medium">Meeting Calendar</li>
          </ol>
        </nav>
        ...
        <div className="flex gap-2 mb-4" role="group" aria-label="Calendar view">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-lg border focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${
              viewMode === "grid" ? "bg-blue-600 text-white border-blue-600" : "bg-white border-gray-300 hover:bg-gray-50"
            }`}
          >
            Grid View
          </button>
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-lg border focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${
              viewMode === "list" ? "bg-blue-600 text-white border-blue-600" : "bg-white border-gray-300 hover:bg-gray-50"
            }`}
          >
            List View
          </button>
        ...
        {isLoading ? (
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
            <table className="w-full table-fixed" role="grid" aria-label={`Calendar for ${MONTHS[month - 1]} ${year}`}>
              <caption className="sr-only">{MONTHS[month - 1]} {year} meeting calendar</caption>
              <thead>
                <tr>
                  {DAYS.map((day) => (
                    <th key={day} scope="col" className="px-1 py-2 text-xs font-semibold text-gray-500 text-center border-b border-gray-200">
                      {day}
                    </th>
                  ))}
                </tr>
                    ...
                    {week.map((day, di) => {
                      ...
                      return (
                        ...
                        >
                          {day !== null && (
                            <>
                              <span className="text-xs font-medium text-gray-600 block mb-1">{day}</span>
          ...
          <ul className="space-y-3 list-none" aria-label="Meetings list">
            {calendar.meetings.length === 0 ? (
              <li className="text-center py-16 text-gray-500">No meetings scheduled for this month.</li>
            ) : (
              calendar.meetings.map((meeting) => {
                ...
                return (
                  <li key={meeting.id} className="meeting-card bg-white border border-gray-200 rounded-lg p-4">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      {meeting.hasPublicArtifacts && (
                        <span className="text-xs text-green-700 flex-shrink-0">📄 Documents available</span>
                      )}
                    </div>
                  </li>
                );
        ...
        {/* Legend */}
        <div className="mt-6 flex flex-wrap gap-4 text-sm text-gray-600" aria-label="Calendar legend">
          <span className="flex items-center gap-1"><span aria-hidden="true">🔵</span> Regular Meeting</span>
          <span className="flex items-center gap-1"><span aria-hidden="true">⚫</span> Cancelled</span>
          <span className="flex items-center gap-1"><span aria-hidden="true">✅</span> Notice Compliant</span>
          <span className="flex items-center gap-1"><span aria-hidden="true">⚠️</span> Late Notice</span>
        </div>
      </main>
    </>
  );
