...
/** Escapes user-controlled strings for safe inclusion in HTML email bodies. */
...
/** Strips newlines and control characters from a string for safe use in email Subject and plain-text fields. */
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await conn.execute<{ member_email: string; status: string }>(
      `SELECT member_email, status FROM gc_team_members WHERE id = ? AND owner_user_id = ?`,
      [memberId, userId]
    );
    ...
    if (result.rows[0].status !== "pending") {
      return res.status(400).json({ error: "Can only resend invite for pending members" });
    }
  ...
  await conn.execute(
    `UPDATE gc_team_members SET invite_token = ?, invited_at = CURRENT_TIMESTAMP WHERE id = ? AND owner_user_id = ?`,
