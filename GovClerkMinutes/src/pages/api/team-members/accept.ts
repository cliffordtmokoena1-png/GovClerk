...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  let row: { id: number; status: string } | null = null;
  try {
    const result = await conn.execute<{ id: number; status: string }>(
      `SELECT id, status FROM gc_team_members WHERE invite_token = ? LIMIT 1`,
      [token]
    );
