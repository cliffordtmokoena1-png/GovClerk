...
async function handler(req: NextRequest) {
  ...
  try {
    ...
    const totalConversations = Number(
      (totalResult.rows[0] as Record<string, unknown>)?.cnt ?? 0
    );
    ...
    const aiHandledCount = Number(
      (aiResult.rows[0] as Record<string, unknown>)?.cnt ?? 0
    );
    ...
    const escalatedCount = Number(
      (escalatedResult.rows[0] as Record<string, unknown>)?.cnt ?? 0
    );
    ...
    const inboundMessages = Number(
      (inboundResult.rows[0] as Record<string, unknown>)?.cnt ?? 0
    );
    ...
    const outboundMessages = Number(
      (outboundResult.rows[0] as Record<string, unknown>)?.cnt ?? 0
    );
    ...
    const pendingScheduled = Number(
      (scheduledResult.rows[0] as Record<string, unknown>)?.cnt ?? 0
    );

    const report: DailyReport = {
      date: today,
      sales: [
        { metric: "Outbound Messages", value: outboundMessages },
      ],
      followUps: [
        { metric: "Pending Scheduled Messages", value: pendingScheduled },
      ],
      scheduledDemos: [],
      totalConversations,
      aiHandledCount,
